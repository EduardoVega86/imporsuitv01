											...
											<div class="col-xs-3">
												<div class="input-group">
													<select id="employee_id" class="form-control" onchange="load(1);">
														...
														<option value="">Todos</option>
														<?php
$sql1 = mysqli_query($conexion, "select * from clientes");
    while ($rw1 = mysqli_fetch_array($sql1)) {
        ?>
															<option value="<?php echo $rw1['id_cliente'] ?>"><?php echo $rw1['nombre_cliente']; ?></option>
															<?php
}
    ?>
													</select>
...
$('.daterange').daterangepicker({
	...
	function load(page){
		...
		$("#loader").fadeIn('slow');
		$.ajax({
			url:'../ajax/rep_ventas_cliente.php',
			data: parametros,
...
<script>
	function reporte(){
		...
		var estado_factura=$("#estado_factura").val();
		var employee_id=$("#employee_id").val();
		VentanaCentrada('../pdf/documentos/rep_ventas_cliente.php?daterange='+daterange+"&estado_factura="+estado_factura+"&employee_id="+employee_id,'Reporte','','800','600','true');
	}
	function reporte_excel(){
		var range=$("#range").val();
		var estado_factura=$("#estado_factura").val();
		var employee_id=$("#employee_id").val();
		window.location.replace("../excel/rep_ventas_cliente.php?range="+range+"&estado_factura="+estado_factura+"&employee_id="+employee_id);
}
</script>
