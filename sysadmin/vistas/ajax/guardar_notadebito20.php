...
if (empty($_POST['id_cliente'])) {
    ...
    $sql_count = mysqli_query($conexion, "select * from tmp_notadebito20 where session_id='" . $session_id . "'");
    ...
    $tipoDocumentodebito = mysqli_real_escape_string($conexion, (strip_tags($_POST["tipoDocumentodebito2"], ENT_QUOTES)));
    ...
    if ($numeroDocMod == '' or $numeroDocMod == null) {
        echo "<script>
        swal({
          title: 'Campo Nro Comprobante vacio ',
          text: 'Intentar nuevamente',
          type: 'error',
          confirmButtonText: 'ok'
        })</script>";
...
// fin de numero de fatura
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        $insert_detail = mysqli_query($conexion, "INSERT INTO motivodebito20 VALUES (NULL,'$id_debito','$numero_factura',NULL,'$cantidad','$desc_tmp','$nombre_producto','$precio_venta_r','$precio_total')");
    ...
    //$numero_factura = '';
    $sql = "INSERT INTO notadebito20 VALUES (null,'$numero_factura','$date_added','$id_cliente','$id_vendedor','$condiciones','$total_factura','$estado','$users','$resibido','1','$id_comp','$trans','$fecha_emision','$tipoDocumentodebito','$fechaDocModificado','$numeroDocMod')";
    $insert = mysqli_query($conexion, $sql);
    generaxmldebito($id_debito);
    $perfil        = mysqli_query($conexion, "select * from perfil");
    $rwperfil         = mysqli_fetch_array($perfil);
    $secuencial_notadebito = '';
    if(isset($rwperfil["secuencialdebito"]) != null){
        $secuencial_notadebito = $rwperfil['secuencialdebito'] + 1;
    }
    $updateperfilsecuencialnotadebito  = mysqli_query($conexion, "UPDATE perfil SET secuencialdebito='" . $secuencial_notadebito . "' WHERE id_perfil='1'"); //Actualizo la nueva cantidad en el inventario
    $ruta_firma = $rwperfil['firma'];
    $pass_firma = $rwperfil['passFirma'];
    $delete = mysqli_query($conexion, "DELETE FROM tmp_notadebito20 WHERE session_id='" . $session_id . "'");
    // SI TODO ESTA CORRECTO
    if ($condiciones == 4) {
        echo "<script>
                swal({
                title: 'DETALLE AL DEBITO GUARDADA CON EXITO CON ATICIPO DE: $simbolo_moneda $resibido_formato',
    ...
    if ($insert_detail) {
        echo "<script>
                    $('#outer_comprobante').load('../ajax/carga_correlativosdebito.php');
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><i class='fa fa-edit'></i> NOTA DEBITO: <?php echo $numero_factura; ?></h4>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="imprimir2" class="btn btn-success btn-block btn-lg waves-effect waves-light" onclick="printFactura('1');" accesskey="p"><span class="fa fa-print"></span> NOTA DEBITO</button>
            </div>
            ...
            <?php 
                ...
                if($rwperfil['autodebito'] == 1 or $rwperfil['autodebito'] == '1'){
                    //$ruta_factura = 'http://localhost/punto_venta/vistas/xml/comprobantes/ND_' . $id_debito . ".xml";
                    $ruta_factura = 'http://'.$_SERVER['HTTP_HOST'].'/vistas/xml/comprobantes/ND_' . $id_debito . ".xml";
                    $ruta = 'http://'.$_SERVER['HTTP_HOST'].'/vistas/xml/firmas/'.$ruta_firma;
                    $ruta_certificado =  $ruta;
                    $pass = $pass_firma;
                    $ruta_respuesta='';
                    echo ' <script>obtenerComprobanteFirmado_sri("' . $ruta_certificado . '","' . $pass . '","' .$ruta_respuesta. '","' .$ruta_factura.'","' .$id_debito.'","NOTA DEBITO","1")</script>';
                }
