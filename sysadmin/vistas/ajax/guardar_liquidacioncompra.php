...
if (empty($_POST['id_cliente'])) {
    ...
    $sql_count = mysqli_query($conexion, "select * from tmp_liquidacioncompra where session_id='" . $session_id . "'");
    ...
    if ($fecha_emision == '' or $fecha_emision == null) {
        echo "<script>
        swal({
          title: 'Campo Fecha Emision vacio ',
          text: 'Intentar nuevamente',
          type: 'error',
          confirmButtonText: 'ok'
        })</script>";
...
//Seleccionamos el ultimo compo numero_fatura y aumentamos una
    $sql        = mysqli_query($conexion, "select LAST_INSERT_ID(id_factura) as last from liquidacioncompra order by id_factura desc limit 0,1 ");
    $rw         = mysqli_fetch_array($sql);
    if(isset($rw["last"]) == null){
        $id_liquidacion = 0 + 1;
    }else{
        $id_liquidacion = $rw['last'] + 1;
    }
...
// fin de numero de fatura
    ...
    $sum_total     = 0;
    $t_iva         = 0;
    $sql           = mysqli_query($conexion, "select * from productos, tmp_liquidacioncompra where productos.id_producto=tmp_liquidacioncompra.id_producto and tmp_liquidacioncompra.session_id='" . $session_id . "'");
    while ($row = mysqli_fetch_array($sql)) {
        ...
        $insert_detail = mysqli_query($conexion, "INSERT INTO liquidacioncomprahasproducto VALUES (NULL,'$id_liquidacion','$numero_factura','$id_producto','$cantidad','$desc_tmp','$precio_venta_r','$precio_total')");
...
// FIN
    ...
    //$numero_factura = '';
    $sql = "INSERT INTO liquidacioncompra VALUES (null,'$numero_factura','$date_added','$id_cliente','$id_vendedor','$condiciones','$total_factura','$estado','$users','$resibido','1','$id_comp','$trans','$fecha_emision')";
    $insert = mysqli_query($conexion, $sql);
    generaxmlliquidacion($id_liquidacion);
    $perfil        = mysqli_query($conexion, "select * from perfil");
    $rwperfil         = mysqli_fetch_array($perfil);
    $secuencial_liquidacion = '';
    if(isset($rwperfil["secuencialliquidacion"]) != null){
        $secuencial_liquidacion = $rwperfil['secuencialliquidacion'] + 1;
    }
    $updateperfilsecuencialnotacredito  = mysqli_query($conexion, "UPDATE perfil SET secuencialliquidacion='" . $secuencial_liquidacion . "' WHERE id_perfil='1'"); //Actualizo la nueva cantidad en el inventario
    $ruta_firma = $rwperfil['firma'];
    $pass_firma = $rwperfil['passFirma'];
    $delete = mysqli_query($conexion, "DELETE FROM tmp_liquidacioncompra WHERE session_id='" . $session_id . "'");
    // SI TODO ESTA CORRECTO
    if ($condiciones == 4) {
        echo "<script>
        swal({
          title: 'LIQUIDACION COMPRA GUARDADA CON EXITO CON ATICIPO DE: $simbolo_moneda $resibido_formato',
    ...
    if ($insert_detail) {
        echo "<script>
        $('#outer_comprobante').load('../ajax/carga_correlativosliquidacion.php');
        $('#resultados5').load('../ajax/carga_num_trans.php')
            $('#modal_vuelto').modal('show');
        </script>";
        $messages[] = "Liquidacion compra  ha sido Guardada satisfactoriamente.";
    } else {
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><i class='fa fa-edit'></i> LIQUIDACI&Oacute;N COMPRA: <?php echo $numero_factura; ?></h4>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="imprimir2" class="btn btn-success btn-block btn-lg waves-effect waves-light" onclick="printFactura('1');" accesskey="p"><span class="fa fa-print"></span>  LIQ COMPRA</button>
            </div>
            ...
            <?php 
                ...
                if($rwperfil['autoliquidacion'] == 1 or $rwperfil['autoliquidacion'] == '1'){
                    /*$ruta_factura = 'http://localhost/punto_venta/vistas/xml/comprobantes/LC_' . $id_liquidacion . ".xml";
                    $ruta = 'http://localhost/punto_venta/vistas/xml/firmas/'.$ruta_firma;sss*/
                    ...
                    $pass = $pass_firma;
                    $ruta_respuesta='';
                    echo ' <script>obtenerComprobanteFirmado_sri("' . $ruta_certificado . '","' . $pass . '","' .$ruta_respuesta. '","' .$ruta_factura.'","' .$id_liquidacion.'","LIQUIDACION COMPRA","1")</script>';
                }
