...
if (!empty($id) and !empty($cantidad) and !empty($precio_venta)) {
    ...
    $comprobar = mysqli_query($conexion, "select * from tmp_notacredito, productos where productos.id_producto = tmp_notacredito.id_producto and tmp_notacredito.id_producto='" . $id . "' and tmp_notacredito.session_id='" . $session_id . "'");
...
// condicion si el stock e menor que la cantidad requerida
        if ($cant > $row['stock_producto'] and $inv == 0) {
            ...
            $sql          = "UPDATE tmp_notacredito SET cantidad_tmp='" . $cant . "', precio_tmp='" . $precio_venta . "' WHERE id_producto='" . $id . "' and session_id='" . $session_id . "'";
...
// condicion si el stock e menor que la cantidad requerida
        if ($cantidad > $stock and $inv == 0) {
            ...
            $insert_tmp = mysqli_query($conexion, "INSERT INTO tmp_notacredito (id_producto,cantidad_tmp,precio_tmp,desc_tmp,session_id) VALUES ('$id','$cantidad','$precio_venta','0','$session_id')");
            echo "<script> $.Notification.notify('success','bottom center','NOTIFICACIÓN', 'PRODUCTO AGREGADO A LA FACTURA CORRECTAMENTE')</script>";
        }
        // fin codicion cantaidad
    }
