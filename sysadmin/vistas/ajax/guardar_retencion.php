...
if (empty($_POST['id_cliente'])) {
    ...
    $fechaemisionretencion = $_POST['fechaEmision'];
    $fechaemisionretencion = date("Y-m-d H:i:s", strtotime($fechaemisionretencion));
    $periodoFiscal = $_POST['periodoFiscal'];
    
    /*$numeroDocMod = $_POST['numeroDocMod'];
    $tipoDocumentodebito = $_POST['tipoDocumentodebito'];
    $subTotalSinImpuesto = $_POST['subTotalSinImpuesto2'];$subTotal122 = $_POST['subTotal122'];
    ...
    $sql = "INSERT INTO retencion VALUES (null,'$numero_factura','$date_added','$id_cliente','$id_vendedor','$condiciones','$valorTotal','$estado','$users',null,'1','$id_comp',null,'$fechaemisionretencion','$periodoFiscal',null,null,null)";
    ...
    $sql        = mysqli_query($conexion, "select LAST_INSERT_ID(id_retencion) as last from retencion order by id_retencion desc limit 0,1 ");
    ...
    $query = mysqli_query($conexion, "SELECT id_retencion from retencion ORDER BY id_retencion DESC")
        or die('error: ' . mysqli_error($conexion));
    $data = mysqli_fetch_assoc($query);
    
    ///insert Notacreditohasproducto
    $idretencion = $data['id_retencion'];

    $codImpuesto = $_POST['tipoImpuesto'];
    ...
    foreach ($codImpuesto as $clave => $valor) {
        $codigo = $codImpuesto[$clave];
        $codigoretencion = $codRetencion[$clave];
        $baseimponible = $baseImponible[$clave];
        $porcentajeretener = $porcentaje[$clave];
        $total = round(floatval($baseimponible) * floatval($porcentajeretener) / 100, 2);
        $numdocsustento = $documento[$clave];
        $fechafinal = $fecha[$clave];
        $fechaDocSustento = str_replace("/", "-", $fecha[$clave]);
        $fechaFormateada = new \DateTime($fechaDocSustento);
        //$fechaemisiondocsustento = $fecha[$clave];

        $coddocsustento = $tipoDoc[$clave];
        //var_dump($coddocsustento);die;

        $query2 = mysqli_query($conexion, "INSERT INTO impuestocomprobanteretencion (retencion_id, codigo, codigoretencion, baseimponible, porcentajeretener, valorretenido, numdocsustento, fechaemisiondocsustento, coddocsustento)
            VALUES('$idretencion', '$codigo', '$codigoretencion', '$baseimponible', '$porcentajeretener', '$total', '$numdocsustento', '$fechafinal', '$coddocsustento')")
            or die('error: ' . mysqli_error($conexion));
    }
    ...
    header("location: ../html/new_retencion.php");
}
