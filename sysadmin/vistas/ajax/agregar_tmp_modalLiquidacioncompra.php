...
if (!empty($id) and !empty($cantidad) and !empty($precio_venta)) {
    ...
    $comprobar = mysqli_query($conexion, "select * from tmp_liquidacioncompra, productos where productos.id_producto = tmp_liquidacioncompra.id_producto and tmp_liquidacioncompra.id_producto='" . $id . "' and tmp_liquidacioncompra.session_id='" . $session_id . "'");
...
// condicion si el stock e menor que la cantidad requerida
        if ($cant > $row['stock_producto'] and $inv == 0) {
            ...
            $('#resultados').load('../ajax/agregarliquidacioncompra_tmp.php');
            </script>";
            exit;
        } else {
            $sql          = "UPDATE tmp_liquidacioncompra SET cantidad_tmp='" . $cant . "', precio_tmp='" . $precio_venta . "' WHERE id_producto='" . $id . "' and session_id='" . $session_id . "'";
...
// condicion si el stock e menor que la cantidad requerida
        if ($cantidad > $stock and $inv == 0) {
            ...
            $insert_tmp = mysqli_query($conexion, "INSERT INTO tmp_liquidacioncompra (id_producto,cantidad_tmp,precio_tmp,desc_tmp,session_id) VALUES ('$id','$cantidad','$precio_venta','0','$session_id')");
            echo "<script> $.Notification.notify('success','bottom center','NOTIFICACIÓN', 'PRODUCTO AGREGADO A LA FACTURA CORRECTAMENTE')</script>";
        }
        // fin codicion cantaidad
    }
...
if (isset($_GET['id'])) //codigo elimina un elemento del array
{
    ...
    $delete = mysqli_query($conexion, "DELETE FROM tmp_liquidacioncompra WHERE id_tmp='" . $id_tmp . "'");
}
