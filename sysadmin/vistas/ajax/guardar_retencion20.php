...
if (empty($_POST['id_cliente'])) {
    ...
    $sql_count = mysqli_query($conexion, "select * from tmp_retencion20 where session_id='" . $session_id . "'");
    ...
    foreach ($dt as $clave->$valor) {
        var_dump($clave,$valor);
    }*/
    $periodoFiscal = mysqli_real_escape_string($conexion, (strip_tags($_POST["periodoFiscal2"], ENT_QUOTES)));
    if ($periodoFiscal == '' or $periodoFiscal == null) {
        echo "<script>
        swal({
          title: 'Campo Periodo Fiscal vacio ',
          text: 'Intentar nuevamente',
          type: 'error',
          confirmButtonText: 'ok'
        })</script>";
...
// fin de numero de fatura
    ...
    while ($row = mysqli_fetch_array($sql)) {
        ...
        $tipoDoc   = $row['tipoDoc'];
        $fecha   = $row['fecha'];
        $total2 = round(floatval($baseImponible) * floatval($porcentaje) / 100, 2);
        ...
        $insert_detail = mysqli_query($conexion, "INSERT INTO impuestocomprobanteretencion20 VALUES (NULL,'$id_retencion','$numero_factura',NULL,'$cantidad','$desc_tmp','$nombre_producto','$precio_venta_r','$precio_total','$tipoImpuesto','$codRetencion','$baseImponible','$porcentaje','$total2','$documento','$fecha','$tipoDoc')");
        //var_dump('vamos biensss',$insert_detail,$sql);die;
        ...
        /*$sql2    = mysqli_query($conexion, "select * from tmp_notadebito20 where id_debito='" . $id_debito . "'");
    ...
    //$numero_factura = '';
    $sql = "INSERT INTO retencion20 VALUES (null,'$numero_factura','$date_added','$id_cliente','$id_vendedor','$condiciones','$total_factura','$estado','$users','$resibido','1','$id_comp','$trans','$fecha_emision','$periodoFiscal')";
    $insert = mysqli_query($conexion, $sql);
    generaxmlretencion($id_retencion);
    $perfil        = mysqli_query($conexion, "select * from perfil");
    $rwperfil         = mysqli_fetch_array($perfil);
    $secuencial_retencion = '';
    if(isset($rwperfil["secuencialretencion"]) != null){
        $secuencial_retencion = $rwperfil['secuencialretencion'] + 1;
    }
    $updateperfilsecuencialretencion  = mysqli_query($conexion, "UPDATE perfil SET secuencialretencion='" . $secuencial_retencion . "' WHERE id_perfil='1'"); //Actualizo la nueva cantidad en el inventario
    $ruta_firma = $rwperfil['firma'];
    $pass_firma = $rwperfil['passFirma'];
    $delete = mysqli_query($conexion, "DELETE FROM tmp_retencion20 WHERE session_id='" . $session_id . "'");
    // SI TODO ESTA CORRECTO
    if ($condiciones == 4) {
        echo "<script>
                swal({
                title: 'DETALLES A LA RETENCION GUARDADA CON EXITO CON ATICIPO DE: $simbolo_moneda $resibido_formato',
    ...
    if ($insert_detail) {
        echo "<script>
                    $('#outer_comprobante').load('../ajax/carga_correlativosretencion.php');
                    $('#resultados5').load('../ajax/carga_num_trans.php')
                $('#modal_vuelto').modal('show');
            </script>";
        //$messages[] = "Nota Debito  ha sido Guardada satisfactoriamente.";
    } else {
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            <div class="modal-header">
                ...
                <h4 class="modal-title" id="myModalLabel"><i class='fa fa-edit'></i> RETENCION: <?php echo $numero_factura; ?></h4>
            </div>
            ...
            <div class="modal-footer">
                ...
                <button type="button" id="imprimir2" class="btn btn-success btn-block btn-lg waves-effect waves-light" onclick="printFactura('1');" accesskey="p"><span class="fa fa-print"></span> RETENCION</button>
            </div>
            ...
            <?php 
                if($rwperfil['autoretencion'] == 1 or $rwperfil['autoretencion'] === '1'){
                    //$ruta_factura = 'http://localhost/punto_venta/vistas/xml/comprobantes/ND_' . $id_debito . ".xml";
                    $ruta_factura = 'http://'.$_SERVER['HTTP_HOST'].'/vistas/xml/comprobantes/RC_' . $id_retencion . ".xml";
                    $ruta = 'http://'.$_SERVER['HTTP_HOST'].'/vistas/xml/firmas/'.$ruta_firma;
                    $ruta_certificado =  $ruta;
                    $pass = $pass_firma;
                    $ruta_respuesta='';
                    echo ' <script>obtenerComprobanteFirmado_sri("' . $ruta_certificado . '","' . $pass . '","' .$ruta_respuesta. '","' .$ruta_factura.'","' .$id_retencion.'","RETENCION","1")</script>';
                }
