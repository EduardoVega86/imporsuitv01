...
if (empty($_POST['id_cliente'])) {
    ...
    $sql_count = mysqli_query($conexion, "select * from tmp_guia where session_id='" . $session_id . "'");
    ...
    $razonsocial = mysqli_real_escape_string($conexion, (strip_tags($_POST["razonsocial2"], ENT_QUOTES)));
    $ructransportista = mysqli_real_escape_string($conexion, (strip_tags($_POST["ructransportista2"], ENT_QUOTES)));
    $placa = mysqli_real_escape_string($conexion, (strip_tags($_POST["placa2"], ENT_QUOTES)));
    ...
    $direccionpartida = mysqli_real_escape_string($conexion, (strip_tags($_POST["direccionpartida2"], ENT_QUOTES)));
    $translado = mysqli_real_escape_string($conexion, (strip_tags($_POST["translado2"], ENT_QUOTES)));
...
//Seleccionamos el ultimo compo numero_fatura y aumentamos una
    $sql        = mysqli_query($conexion, "select LAST_INSERT_ID(id_factura) as last from guia order by id_factura desc limit 0,1 ");
...
// fin de numero de fatura
    ...
    $sum_total     = 0;
    $t_iva         = 0;
    $sql           = mysqli_query($conexion, "select * from productos, tmp_guia where productos.id_producto=tmp_guia.id_producto and tmp_guia.session_id='" . $session_id . "'");
    while ($row = mysqli_fetch_array($sql)) {
        ...
        $insert_detail = mysqli_query($conexion, "INSERT INTO guiahasproducto VALUES (NULL,'$id_factura','$numero_factura','$id_producto','$cantidad','$desc_tmp','$precio_venta_r','$precio_total')");
...
// FIN
    ...
    $sql = "INSERT INTO guia VALUES (null,'$numero_factura','$date_added','$id_cliente','$id_vendedor','$condiciones','$total_factura','$estado','$users','$resibido','1','$id_comp','$trans','$direccionpartida','$fechainicio','$fechafin','$razonsocial','$ructransportista','$placa','$translado')";
    $insert = mysqli_query($conexion, $sql);
    generaxmlguia($id_factura);
    $perfil        = mysqli_query($conexion, "select * from perfil");
    $rwperfil         = mysqli_fetch_array($perfil);
    $secuencial_guia = '';
    if(isset($rwperfil["secuencialguia"]) != null){
        $secuencial_guia = $rwperfil['secuencialguia'] + 1;
    }
    $updateperfilsecuencialnotacredito  = mysqli_query($conexion, "UPDATE perfil SET secuencialguia='" . $secuencial_guia . "' WHERE id_perfil='1'"); //Actualizo la nueva cantidad en el inventario
    $ruta_firma = $rwperfil['firma'];
    $pass_firma = $rwperfil['passFirma'];
    $delete = mysqli_query($conexion, "DELETE FROM tmp_guia WHERE session_id='" . $session_id . "'");
    ...
    <div class="modal-dialog" role="document">
        <div class="modal-content">
            ...
            <?php 
                ...
                if($rwperfil['autoguia'] == 1 or $rwperfil['autoguia'] == '1'){
                    ...
                    $ruta_factura = 'http://'.$_SERVER['HTTP_HOST'].'/vistas/xml/comprobantes/GR_' . $id_factura . ".xml";
                    $ruta = 'http://'.$_SERVER['HTTP_HOST'].'/vistas/xml/firmas/'.$ruta_firma;
                    $ruta_certificado =  $ruta;
                    $pass = $pass_firma;
                    $ruta_respuesta='';
                    echo ' <script>obtenerComprobanteFirmado_sri("' . $ruta_certificado . '","' . $pass . '","' .$ruta_respuesta. '","' .$ruta_factura.'","' .$id_factura.'","GUIA","1")</script>';
                }
