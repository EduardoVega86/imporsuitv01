...
if ($action == 'ajax') {
    ...
    $query = mysqli_query($conexion, "SELECT icr.codigo AS codigo, icr.codigoretencion AS codigor, SUM(icr.baseimponible) AS base, SUM(icr.valorretenido) AS valor, icr.numdocsustento AS numdocsustento
                                        FROM impuestocomprobanteretencion20 icr
                                        INNER JOIN retencion20 r ON r.id_factura = icr.id_retencion
                                        where $sWhere LIMIT $offset,$per_page");
        ...
        <div class="table-responsive">
            <table class="table table-condensed table-hover table-striped table-sm">
                <tr>
                    ...
                    <th class='text-center'>Numero del Documento</th>
...
$finales = 0;
        while ($row = mysqli_fetch_array($query)) {
            ...
            if($codigo  != null){
                ...
                $base    = $row["base"];
                $valor    = $row["valor"];
                $numdocsustento    = $row["numdocsustento"];
                if($tiporetencion == '1'){
                    $tiporetencion = 'RENTA';
                ...
                ?>
                        <tr>
                            ...
                            <td class='text-center'><?php echo $numdocsustento; ?></td>
