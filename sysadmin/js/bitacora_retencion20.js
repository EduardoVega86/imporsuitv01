		...
		function load(page) {
		    ...
		    $("#loader").fadeIn('slow');
		    $.ajax({
		        url: '../ajax/buscar_retencion20.php?action=ajax&page=' + page + '&q=' + q,
		...
		function generarXML(id_factura) {
			var mywindow = VentanaCentrada('../xml/documentos/generar_xmlretencion20.php?id_factura=' + id_factura, 'Factura', '', '724', '568', 'true');
		...
		$("#enviarcorreo").submit(function(event) {
			...
			//alert($('#obteneridcliente').val());
			$('#iddelcliente').val($('#obteneridcliente').val());
			
			var parametros = $(this).serialize();
			$.ajax({
				type: "POST",
				url: "../ajax/enviarcorreocredito.php",
				data: parametros,
				beforeSend: function(objeto) {
